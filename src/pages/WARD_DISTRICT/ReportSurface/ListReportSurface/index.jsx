...
const ReportSpaces = () => {
    ...
    const loadSpacesByDistrict = async () => {
        const {data} = await axiosService.get(`/reports-surface/area?=${current_user?.district?.id}`);
        return data;
    }

    const loadReportSpacesByWard = async () => {
        const {data} = await axiosService.get(`/reports-surface/area?ward=${current_user?.ward?.id}`);
        return data;
    }
    useEffect(() => {
        if(current_user?.role === "DISTRICT_MANAGER"){
            loadSpacesByDistrict().then((data) => {
                            ...
                            <tr key={index} className="border-top">
                                ...
                                <td>
                                    {hiddenActionEditByReportState(tdata?.state) ? <div></div>
                                        : <Link to={`/them-yeu-cau-chinh-sua-bang-quang-cao/${tdata.id}`}>
