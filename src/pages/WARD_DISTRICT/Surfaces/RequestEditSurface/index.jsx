...
const RequestEditSurfaces = () => {
  ...
  const handleRequestEditSurfaces = async (params) => {
    const response = await axiosService.post("/temp-surface/condition-update", params, {
      headers: {
  ...
  const handleSubmit = (values, {setSubmitting}) => {
    ...
    handleRequestEditSurfaces(formData).then((response) => {
      const {status} = response;
      if (status === 200 || status === 201) {
        Swal.fire({
          icon: 'success',
          title: 'Yêu cầu chỉnh sửa bảng quảng cáo thành công',
  ...
  const handleSurfaceById = async () => {
    const response = await axiosService.get(`temp-surface/${surfaceId}`);
    return response;
  }

  useEffect(() => {
    handleSurfaceById().then((response) => {
      const {data, status} = response;
      if(status === 200 || status === 201) {
        formikRef.current.setFieldValue("reason", data?.reason)
        formikRef.current.setFieldValue("imgUrl", data?.imgUrl)
        formikRef.current.setFieldValue("expiredDate", data?.expiredDate)
        formikRef.current.setFieldValue("height", data?.height)
        formikRef.current.setFieldValue("widght", data?.width)
        formikRef.current.setFieldValue("surfaceType", data?.surfaceType?.id)
        formikRef.current.setFieldValue("space", data?.space?.id)
  ...
  return (
      <>
        <Container>
          <h3 className={"fw-bolder"}>Tạo yêu cầu chỉnh sửa quảng cáo mới</h3>
...
export default RequestEditSurfaces;